        ...
        for i, dl in enumerate(val_loader):
            ...
            # if (i+1) % args.wandb_interval == 0:
                if args.wandb_log:
                    wandb.log({"iter": i, 
                               "gstep": global_step, 
                               args.corruption: top1.avg, 
                               f'{args.corruption}/batch': acc1, 
                               "accuracy": top1.avg}, commit=True)
                    # wandb.log({"gstep": global_step, "accuracy": top1.avg}, commit=True)
                    # wandb.log({f'{args.corruption}/top1': top1.avg,
                    #            f'{args.corruption}/top5': top5.avg})
                    # if args.method == 'deyo':
                    #     wandb.log({ f'total_acc_pl_1': total_count_corr_pl_1/total_count_backward,
...
def def_run_deyox(module_name):
    ...
    def _run_deyo_aug(args, net, logger):
        ...
        optimizer = torch.optim.SGD(params, args.lr, momentum=0.9)
        adapt_model = module.DeYOAug(net, args, optimizer, deyo_margin=args.deyo_margin, margin_e0=args.deyo_margin_e0)
...
def run_otent(args, net, logger):
    # src_emb = torch.load('/ssd1/tta/imagenet_val_resnet50_e1k.pth').cuda()
    # src_l1ft = torch.load('/ssd1/tta/imagenet_val_resnet50_l1ft.pth').cuda()
    src_l1ft = torch.load('/ssd1/tta/imagenet_val_resnet50_lyrfts.pth')#.cuda()
...
def run_eata(args, net, logger, aug=False):
    ...
    if not aug:
        ...
        adapt_model = eata_aug.EATAAug(args, net, optimizer, fishers, args.fisher_alpha, e_margin=args.e_margin, d_margin=args.d_margin)
...
def run_deyo_aug(args, net, logger):
    ...
    optimizer = torch.optim.SGD(params, args.lr, momentum=0.9)
    adapt_model = deyo_aug.DeYOAug(net, args, optimizer, deyo_margin=args.deyo_margin, margin_e0=args.deyo_margin_e0)
...
def main(args):     
    ...
    # assert args.method in ['tent', 'eata', 'sar', 'deyo', 'no_adapt', 'ot', 'bn', 'tentb', 'stent']
    ...
    # common_corruptions = ['glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog', 'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression']
    # common_corruptions = ['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur', 'glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost']
    # common_corruptions = ['fog', 'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression']
    # common_corruptions = ['elastic_transform', 'pixelate', 'jpeg_compression']
    # common_corruptions = ['fog', 'brightness', 'contrast']
    ...
    if args.wandb_log:
        ...
        wandb.run.name = f"{args.exp_name}/{rn_method}/" + "_".join([f'{k}{v}' for k, v in rn_dict.items()])
    ...
    for corrupt_i, corrupt in enumerate(tqdm(common_corruptions)):
        ...
        if args.method == "tent":
            ...
            # adapt_model = run_deyo_aug(args, net, logger)
